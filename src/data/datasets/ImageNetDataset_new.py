...
class ImageNetDataset(Dataset):
    def __init__(self, 
        ...
        elif split in ["100-10percent", "100-1percent"]:
            with open(f"src/data/datasets/imagenet-100_classes.txt") as f:
                classes = f.read().strip().split()
                if split == "100-1percent": images_file = "src/data/datasets/imagenet_1percent.txt"
                if split == "100-10percent": images_file = "src/data/datasets/imagenet_10percent.txt"
                with open(images_file) as f:
                    for file in f.readlines():
                        synset = file.split('_')[0]
                        if synset in classes:
                            self.image_paths.append(os.path.join(self.root, synset, file.strip()))
                            self.labels.append(classes.index(synset))
                self.classes = [self.synset_to_label[cls][1] for cls in classes]
        elif self.split == "100":
            with open(f"src/data/datasets/imagenet-100_classes.txt") as f:
                classes = f.read().strip().split()
                for i, imclass in enumerate(classes):
                ...
                self.classes = [self.synset_to_label[cls][1] for cls in classes]
        elif self.split in ["val", "100-val"]:
            self.root = os.path.join(root, 'val')
            self.mapping = json.load(open(self.root[:self.root.index("/ILSVRC/Data/CLS-LOC/val")] + "/imagenet_val_labels.json"))
            self.image_ids = list(self.mapping.keys())
            if self.split == "100-val":
                with open(f"src/data/datasets/imagenet-100_classes.txt") as f:
                    classes = f.read().strip().split()
                    for img_id in self.image_ids:
                        synset = self.mapping[img_id]["synset_id"]
                        if synset in classes:
                            self.image_paths.append(os.path.join(self.root, f"{img_id}.JPEG"))
                            self.labels.append(classes.index(synset))
                    self.classes = [self.synset_to_label[cls][1] for cls in classes]
            else:
                raise NotImplementedError("Val set to be implemented...")
        else:
            raise ValueError("Split must be one of ['train', '1percent', '10percent', '100', '100-10percent', 'val', '100-val'].")
