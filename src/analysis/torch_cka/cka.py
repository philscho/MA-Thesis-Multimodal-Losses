...
class CKA:
    ...
    def compare(self,
        ...
        # for (x1, y1), (x2, y2) in tqdm(zip(dataloader1, dataloader2), desc="| Comparing features |", total=num_batches):

            self.model1_features = {}
            self.model2_features = {}
            _ = self.model1(x1.to(self.device))
            _ = self.model2(x2.to(self.device))
            # _ = self.model2(y1.to(self.device))

            for i, (name1, feat1) in enumerate(self.model1_features.items()):
                # print(name1)
                # print(feat1)
                if type(feat1) is tuple:
                    feat1 = feat1[0]  # hidden states are first element of tuple
                X = feat1.flatten(1)
                #X = feat1.flatten(1)
                K = X @ X.t()
                ...
                for j, (name2, feat2) in enumerate(self.model2_features.items()):
                    if type(feat2) is tuple:
                        feat2 = feat2[0]  # hidden states are first element of tuple
                    Y = feat2.flatten(1)
                    L = Y @ Y.t()
                    L.fill_diagonal_(0)
